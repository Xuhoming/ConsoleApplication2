...
const float yMax = -0.18624;// -0.218759;
...
int main()
{
	const int numberOfViews = 5;//点云数量
	...
	//std::string extension(".pcd");										  // 通过遍历文件名，读取pcd文件
	for (int i = 0; i < numberOfViews; i++) //遍历所有的文件名
	{
		//std::string fname = prefix + num2str(i);
	std::string fname = prefix + num2str(i) + extension;
		...
		for (int j = 0; j < m.cloudWithNormal->size(); ++j)  //读取PCD m中的x，y，z，保存并弹入名叫data 的vector数组
		{
			m.mass(0) += m.cloudWithNormal->points[j].x;
			m.mass(1) += m.cloudWithNormal->points[j].y;
			m.mass(2) += m.cloudWithNormal->points[j].z;
	...
	}// ?mass的用处 什么是点云的质心  为什么所有点云的质心等于 点云质心乘法线指针最后除所有点云的个数
	...
	*Back += *data[5].cloudWithNormal;
	...
	float cos_angle = cos(M_PI * 10 / 180);// ？什么意思 180度乘10除以180度
	//创建一个AABB(axis-aligned bounding box)的包围盒
