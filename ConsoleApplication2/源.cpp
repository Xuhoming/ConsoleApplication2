...
int main()
{
	...
	//pcl::io::savePCDFile("test.pcd", data[1], false);
	...
	*Back += *data[5].cloudWithNormal;
	UpdatePCDMass(data[4]);
	UpdatePCDMass(data[5]);
	pcl::io::savePCDFile("test1.pcd", *Front, false);
	// ----------------------------------------------------
	...
	float cos_angle = cos(M_PI*10/180 ); //12
	...
	for (int iter = 0; iter < iteration; ++iter)
	{
		...
		//cout<<"counter_Back"<< counter_Back<<endl;
		...
		rejector.setThreshold(M_PI*10/180); //12
